...
function renderFooter() {
  ...
  footerContainer.innerHTML = `
    <footer class="footer-dark">
      <section class="footer-dark-inner">
        <section class="footer-brand-col">
          <p class="footer-dark-brand">
            <span class="brand-icon" style="width:30px; height:30px; font-size:15px;">TB</span>
            TutorBridge
          </p>
...
function renderFAB() {
  ...
  fab.setAttribute('aria-label', 'My Profile & Billing');
...
function setupGlobalModals() {
  if (!document.getElementById('faq-modal-overlay')) {
    const faqModal = document.createElement('section');
    ...
    faqModal.innerHTML = `
      <section class="modal-card">
        ...
        <ul id="faq-accordion-container" style="display:flex; flex-direction:column; gap:10px; list-style:none; margin:0; padding:0;"></ul>
  ...
  if (!document.getElementById('feedback-modal-overlay')) {
    ...
    feedbackModal.innerHTML = `
      <section class="modal-card">
        ...
        <span class="stars-select" id="feedback-stars" style="display:flex; justify-content:center; gap:8px; font-size:28px; color:var(--amber); margin:12px 0; cursor:pointer;">
...
function openFAQModal() {
  ...
  if (container && typeof FAQ_DATA !== 'undefined') {
    container.innerHTML = FAQ_DATA.map((item, index) => `
      <li style="border:1px solid var(--grey-mid); border-radius:8px; overflow:hidden;">
        ...
        <p id="faq-ans-${index}" style="display:none; padding:12px; font-size:12.5px; color:var(--charcoal); background:#fff; border-top:1px solid var(--grey-mid);">
          ${item.answer}
        </p>
      </li>
    `).join('');
